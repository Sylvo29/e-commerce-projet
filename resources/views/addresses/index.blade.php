...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.address.create') }}" class="btn btn-success m-1">
            ...
            <tbody>
                @foreach($addresses as $address)
						<tr><td>{{ $address->id }}</td>
							...
							<td>{{ $address->user }}</td>
						<td>
                    <a href="{{ route('admin.address.show', ['id' => $address->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.address.edit', ['id' => $address->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $address->id }}" class="btn btn-danger btn-sm deleteBtn">
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/addresses/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/addresses/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Address th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Address');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Address');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Address', JSON.stringify(selectedColumns));
        }
        ...
        function sortTable(columnIndex) {
            const table = document.getElementById('Address');
