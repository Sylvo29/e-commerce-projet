...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.method.create') }}" class="btn btn-success m-1">
            ...
            <thead>
                <tr>
                    <th scope="col">N#</th>
						...
						<th scope="col">Test_public_key</th>
						<th scope="col">Test_private_key</th>
						<th scope="col">Prod_public_key</th>
						<th scope="col">Prod_private_key</th>
            ...
            <tbody>
                @foreach($methods as $method)
						<tr><td>{{ $method->id }}</td>
							...
							<td>{{ $method->prod_private_key }}</td>
							    <td>
    <div class="form-check form-switch">
        <input name="isAvailable" id="isAvailable" data-id="{{$method->id}}" value="true" data-bs-toggle="toggle"  {{ isset($method) && $method->isAvailable == 'true' ? 'checked' : '' }} class="form-check-input" type="checkbox" role="switch" />
    </div>
</td>
						<td>
                    <a href="{{ route('admin.method.show', ['id' => $method->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.method.edit', ['id' => $method->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $method->id }}" class="btn btn-danger btn-sm deleteBtn">
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/methods/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/methods/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Method th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Method');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Method');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Method', JSON.stringify(selectedColumns));
        }
        ...
        function sortTable(columnIndex) {
            const table = document.getElementById('Method');
