...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.orderdetails.create') }}" class="btn btn-success m-1">
...
<div class="">
    ...
    <div class="table-responsive">
        <table  id="Orderdetails" class="table">
            <thead>
                <tr>
                    <th scope="col">N#</th>
						...
						<th scope="col">Sub_total_ht</th>
						<th scope="col">Sub_total_ttc</th>
            ...
            <tbody>
                @foreach($orderdetails as $orderdetails)
						<tr><td>{{ $orderdetails->id }}</td>
							<td>{{ $orderdetails->product_name }}</td>
							<td>{{ $orderdetails->product_description }}</td>
							<td>{{ number_format($orderdetails->soldePrice, 2, ',', ' ') . ' €' }}</td>
							<td>{{ number_format($orderdetails->regularPrice, 2, ',', ' ') . ' €' }}</td>
							<td>{{ $orderdetails->quantity }}</td>
							<td>{{ $orderdetails->taxe }}</td>
							<td>{{ $orderdetails->sub_total_ht }}</td>
							<td>{{ $orderdetails->sub_total_ttc }}</td>
						<td>
                    <a href="{{ route('admin.orderdetails.show', ['id' => $orderdetails->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.orderdetails.edit', ['id' => $orderdetails->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $orderdetails->id }}" class="btn btn-danger btn-sm deleteBtn">
        ...
        <div class="d-flex justify-content-center">
            {{ $paginateOrderdetails->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
</div>
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/orderdetails/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/orderdetails/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Orderdetails th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Orderdetails');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Orderdetails');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Orderdetails', JSON.stringify(selectedColumns));
        }
